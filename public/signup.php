...
if(isset($_GET['session_id']) && !empty($_GET['session_id'])){
        ...
        $sql='update users set username = SUBSTR(email_work,5), email = SUBSTR(email,5) where username ='.$fd_id;
        $db->query($sql);
...
if(isset($_POST['-fd_id']) && !empty($_POST['-fd_id'])){
        ...
        $user_id=$_POST['-fd_id'];
        $customField['signedup'] = (isset($company['subscription_id']) && $company['subscription_id']? false : true);
        ...
        if (isset($fd_id) && !empty($fd_id)){
            ...
            if(!empty($company['signup_subscription'])){
                ...
                $re = $db->query('delete from users where username='.$fd_id.' and fd_id = '.$fd_id);
                ...
                $_SESSION['subscription']['email'] = $email;
                ...
                $_SESSION['subscription']['success_url'] = "https://".$_SERVER['SERVER_NAME'] . '/signup.php?session_id={CHECKOUT_SESSION_ID}';
                $_SESSION['subscription']['cancel_url'] = "https://".$_SERVER['SERVER_NAME'];
                $_SESSION['subscription']['config'] = $company['stripe'];
                // . '/signup.php?cancel_id='.$_SESSION['subscription']['fd_id'].'-'.$_SESSION['subscription']['user_id']
                $gotoStripe = TRUE;
                ...
                $_SESSION['JUSTSIGNED']=1;
                Redirect::to('login.php?s=1&u='.$fd_id);
            }
        }
}
...
<div id="wrapper">
  ...
  <?php
    ...
    ?>
        ...
        <script type="text/javascript">jaxon_initStripe();</script>
    <?php
                            ...
                            <div class="box-static box-border-top p-30">
                                ...
                                <form id="form_account" class="m-0" onsubmit="return jaxon_SignUp(jaxon.getFormValues('form_account'))">
                                    ...
                                    <div class="clearfix">
                                        ...
                                        <?php }?>
                                        <?php if ($lang['FRM_BUILDING_LOCATION']) {?>
                                            <?php $listBuilding = getCompanyBuildingList($company['id']); ?>                              
                                            <div class="form-group">
                                                <label for="building"><?php echo $lang['FRM_BUILDING_LOCATION'] ; ?></label>
                                                <select class='form-control' id='building' name='building'>
                                                <?php foreach ($listBuilding as $building){ ?>
                                                    <option value='<?php echo $building->label ?>'><?php echo $building->label ?></option>
                                                <?php } ?>
                                                </select>                                                
                                            </div>                                            
                                        <?php }?>
